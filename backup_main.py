...
def get_base_path():
    ...
    if getattr(sys, 'frozen', False):
        ...
        return os.path.dirname(sys.executable)  # 获取 exe 文件所在的目录
    return os.path.abspath(".") # 否则，返回当前工作目录的绝对路径
...
def start_show_answer():
    """
    响应“获取答案”按钮点击事件，开始获取并逐渐显示答案。

    此函数禁用开始按钮，清空答案 Label，获取答案文本，
    如果成功获取答案，则开始逐渐显示；否则，显示错误信息并重新启用开始按钮。
    """
    logging.info("用户点击了 '获取答案' 按钮 - 开始逐渐显示答案") # 记录用户点击按钮事件
    answer_label.config(text="") # 清空答案 Label 的文本
    start_button.config(state=tk.DISABLED) # 禁用开始按钮，防止用户重复点击
    ...
    if full_answer_text:
        ...
        answer_label.config(text="未能获取答案，请重试。", foreground='#e74c3c') # 设置答案 Label 显示错误信息，颜色为红色
        logging.error("未能从答案列表中获取有效答案。") # 记录错误信息
...
root.config(menu=menubar, background='#082032')
# ----- 样式配置 -----
